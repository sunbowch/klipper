...
#   fans stops if fan_pwm is below min_speed and e is >= max_delta,
#   meaning that the target_temperature has been reached with a margin.
#   fan starts when measured_temperature is greater than target_
#   temperature + max_delta. max_delta can be tuned to avoid rapid
#   On-Off oscillations of the fan near the reference temperature.
#pid_deriv_time: 2.0
#   A time value (in seconds) over which temperature measurements will
